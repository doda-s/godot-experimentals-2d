...
# Option button secene path
@export var opt_button_scene_path: String
...
# Display the dialogue on the HUD.
...
# Hides the HUD that displays the dialogue. Also clears the title data and dialogue content.
...
func _load_options(options) -> void:
	...
	for opt in options:
		...
		if opt_button_scene_path != "":
			buttom_scene = opt_button_scene_path
...
# Remove all player selection buttons.
